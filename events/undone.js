...
module.exports = {
    async Undone(text, message) {
        console.log("undoned again")
        if (!text[1]) {
            console.log("no pos")
        ...
        const series = text[2]
        const url = `https://deschtimes.com/api/v1/groups/${deschtimesToken}/shows/${series}/staff?finished=false&member=${message.author.id}&position=${position}`
        ...
        if (returnData.ok) {
            message.reply(`${position} marked undone.`)
        } else {
